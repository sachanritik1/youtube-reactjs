...
import { CommentList } from "./CommentsContainer";
import CreateComment from "./CreateComment";

const CommentCard = ({ comment }) => {
  const { content, likes, replies, owner, video } = comment;
  ...
  const getCommentReplies = async () => {
    ...
    console.log(json);
    setReplyList((prev) => [...prev, ...json.data]);
  };
  ...
  return (
    <div>
      <div className="flex shadow-sm bg-border-gray align-center p-1">
        <img
          className="w-8 h-8 m-2 border-2 border-gray-100 rounded-full"
          alt="user-icon"
        ...
        <div>
          ...
          <p>{content}</p>
          <div className="flex">
            <p className="mr-5">{likes} 👍</p>
            <button className="ml-5" onClick={handleShowReply}>
              {replies} {replies < 2 ? "reply" : "replies"}
            </button>
            <CreateComment
              videoId={video}
              parent={comment?._id}
              setComments={setReplyList}
            />
          </div>
        </div>
      </div>
      <div className="ml-10">
        <CommentList comments={replyList} />
      </div>
    </div>
  );
};
